...
def get_video(video):
    ...
    faces, coords = face_detection_wrapper(mtcnn, video, every_n_frames=FACE_FRAMES,
                                           facedetection_downsample=FACEDETECTION_DOWNSAMPLE,
                                           max_frames_to_load=MAX_FRAMES_TO_LOAD)

    if len(faces):
        return faces[:MAX_FACES_HIGHTHRESH], coords[:MAX_FACES_HIGHTHRESH]

    else:
        return "Found no faces!", "Found no faces!"
